...
module AnnotateModels
  ...
  def self.annotate_one_file(file_name, info_block)
    if File.exist?(file_name)
      ...
      # Put new schema info right before the class definition
      if index = content =~ /class[\w ]+<\W+ActiveRecord::Base/
        File.open(file_name, "w") { |f| f.puts content.insert(index, info_block) }
      else
