  ...
  describe "tag filter controls" do
    ...
    def mock_tag(stubs = {})
      ...
      mock_model(Tag, stubs.reverse_merge(
        :name => "Tag 1", :user_id => user.id, :user => user, :positive_count => 0, :negative_count => 0, :classifier_count => 0
      ))
    end
    ...
    it "creates a filter control with a tooltip showing the trining and author information" do
      user = mock_model(User, :login => "craig")
      tag = mock_tag(:user_id => user.id, :user => user, :positive_count => 1, :negative_count => 2, :classifier_count => 3)
